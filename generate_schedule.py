...
def generate_schedule():
    ...
    min_date = df_gantt['Início'].min() - timedelta(days=5)
    max_date = df_gantt['Fim'].max() + timedelta(days=5)

    fig.update_xaxes(
        title="", 
        tickformat="%d %b", 
        dtick="M1", # Month ticks
        side="top",
    ...
    output_html = 'cronograma_sequencial.html'
    fig.write_html(output_html)
    print(f"HTML gerado: {output_html}")
    
    try:
        output_png = 'cronograma_sequencial.png'
        fig.write_image(output_png, width=2000, height=fig.layout.height, scale=1.5)
        print(f"Imagem gerada: {output_png}")
    except Exception as e:
